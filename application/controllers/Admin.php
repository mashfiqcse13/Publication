...
//
        ...
        print_r($this->Stock_manages->book_details_by_id(1));
        echo "\n";

         print_r($this->Stock_manages->contact_details_by_type(1));
...
//        $this->load->model('custom/stock_manage');
    ...
    function memo_management($cmd = false, $primary_id = false) {
        ...
        if ($cmd == 'success')
            $AutoPrintPageOpenCommandJS = 'window.open("' . site_url('admin/memo/' . $primary_id) . '");';
        else
